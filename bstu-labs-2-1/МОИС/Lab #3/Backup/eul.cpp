...
void Eul::find(GrTrav bf, Eulerian::mode mode)
{
	...
	while (true)
	{
		...
		if (next != -1)
		{
			...
			{
				this->cirE[cirE_id][0] = trGraph[e_id][!j];
				this->cirE[cirE_id][1] = trGraph[e_id][j];
				cirE_id++;
			}
			else if (mode == Eulerian::mode::path)
			{
				this->pathE[pathE_id][0] = trGraph[e_id][!j];
				this->pathE[pathE_id][1] = trGraph[e_id][j];
				pathE_id++;
			}
			marked[e_id] = true;
			cur_ver = next;
		...
		else if(bedNext != -1)
		{
			if (mode == Eulerian::mode::circuit)
			{
				this->cirE[cirE_id][0] = trGraph[bedE_id][!j];
				this->cirE[cirE_id][1] = trGraph[bedE_id][j];
				cirE_id++;
			}
			else if (mode == Eulerian::mode::path)
			{
				this->pathE[pathE_id][0] = trGraph[bedE_id][!j];
				this->pathE[pathE_id][1] = trGraph[bedE_id][j];
				pathE_id++;
			}
			marked[bedE_id] = true;
			cur_ver = bedNext;

...
{
	e.push_back(newEdge);
	deg.getAccess(newEdge.fVertex - 1)++;
	deg.getAccess(newEdge.secVertex - 1)++;
	numOfE++;
}
void Graph::addE(const GraphVertex fVertex, const GraphVertex secVertex)
{
	e.push_back(Edge(fVertex, secVertex));
	deg.getAccess(fVertex - 1)++;
	deg.getAccess(secVertex - 1)++;
	numOfE++;
}
void Graph::addE(const GraphVertex fVertex, const GraphVertex secVertex, const GraphEdgeWeight weight)
{
	e.push_back(Edge(fVertex, secVertex, weight));
	deg.getAccess(fVertex - 1)++;
	deg.getAccess(secVertex - 1)++;
	numOfE++;
}
...
//
 // ! there is no checking for correctness of given vertices
Edge::Edge()

...
template<typename T>
Permut<T>::Permut(T *arr, int size)
{
	this->size = size;
...
template<typename T>
Permut<T>::Permut(const Permut& obj)
{
	this->size = obj.size;
	this->arr = new T[this->size];
	for (int i = 0; i < this->size; i++)
...
template<typename T>
void Permut<T>::get(int id, int k, T*&arr)
{
	try
	{
		if (id < 0)
...
template<typename T>
void Permut<T>::get(int id, int k)
{
...
template<typename T>
void Permut<T>::setArr(T* arr, int size)
{
	if (size != 0)
	{
		delete[] this->arr;

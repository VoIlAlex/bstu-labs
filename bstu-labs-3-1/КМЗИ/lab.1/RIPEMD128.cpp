...
std::string RIPEMD128::add_additional_bits(const std::string &input)
{
	...
	for (int i = result.size(); i % 64 != 56; i++)
		result += char(0);
	std::clog << "Additional bits: " << result << std::endl;
	return result;
}
...
std::string RIPEMD128::add_initial_length(const std::string &input)
{
	...
	std::clog << "Initial length: " << result << std::endl;
	return result;
}

void RIPEMD128::log_everything(const std::vector<int> &others) const
{
	std::clog << "H: ";
	for (int i = 0; i < 4; i++)
	{
		if (i != 0)
			std::clog << ' ';
		std::clog << std::hex << h[i];
	}

	std::clog << "\nInitial length: " << std::dec << m_initial_length << std::endl;

	std::clog << "\nOthers: " << std::endl;
	for (int i = 0; i < others.size(); i++)
	{
		std::clog << i << ": " << std::hex << others[i] << std::endl;
	}
}
...
void RIPEMD128::main_cycle(const std::string &M)
{
	...
	for (int i = 0; i < M.size() / 512 + 1; i++)
	{
		...
		C2 = h[2];
		D2 = h[3];
		log_everything({A1, B1, C1, D1, A2, B2, C2, D2, T});
		for (int j = 0; j < 64; j++)
		{
...
std::string RIPEMD128::operator()(const std::string &input)
{
	...
	std::clog << "Result length 1: " << std::dec << input.length() << std::endl;
	std::string result = add_additional_bits(input);
	std::clog << "Result length 2: " << std::dec << result.length() << std::endl;
	result = add_initial_length(result);
	std::clog << "Result length 3: " << std::dec << result.length() << std::endl;
	main_cycle(result);
